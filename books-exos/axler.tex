...
\end{align*}
which leads to the only subspace dimensionless, i.e. $U\cup W = \{0\}$
%------------------------------------------------------------------------ 
\exo{} Following previous reasoning leads to $ \dimension{U \cap W} = 1 \implies U\cap W \neq \{0\}$

%------------------------------------------------------------------------ 
\exo{} Using \tref{it:T2_18}
\begin{align*}
  \dimension{(U_1+ U_2 + U_3) } = \dimension{U_1} +  \dimension{U_2} + \dimension{U_3} - \dimension{(U_2\cap U_3)}  - \dimension{(U_1\cap (U_2+U_3)) }
\end{align*}
If $U_1 \cap (U_2 + U_3) = U_1 \cap U_2 + U_1 \cap U_3 $ \hilight{??} then $\dimension{(U_1 \cap U_2 + U_1 \cap U_3 ) } = \dimension{U_1 \cap U_2} +  \dimension{ U_1 \cap U_3}  - \dimension{( U_1 \cap U_2\cap U_3)} $

%------------------------------------------------------------------------ 
\exo{}  Using \tref{it:T2_18} we have that $\dimension{  \sum U_i  } = \sum \dimension{U_i} - \underbrace{\dimension{ (U_1 \cap \sum_{j>1} U_j) }}_{\ge 0}  $
$\implies  \dimension{  \sum U_i  } \leq \sum \dimension{U_i}  $

%------------------------------------------------------------------------ 

\exo{} Following from previous result, $U_1 \cap \bigoplus\limits_{j>1} U_j = \{0\}$ because the bases of each $U_i$ are l.i. .
